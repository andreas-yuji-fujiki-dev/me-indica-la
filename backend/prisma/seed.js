...
function loadDotEnv() {
    ...
    const lines = fs_1.default.readFileSync(envPath, 'utf-8').split(/\r?\n/);
...
async function main() {
    ...
    const providers = [
        {
            ...
            status: client_1.ProviderStatus.APPROVED,
            isActive: true,
    ...
    for (const city of cities) {
        const record = await prisma.city.upsert({
            ...
            create: { name: city.name, slug: generateSlug(city.name), state: city.state, isActive: true },
        });
    ...
    for (const provider of providers) {
        ...
        const passwordHash = await bcrypt_1.default.hash('Seed123!', 10);
        ...
        const providerRecord = await prisma.provider.upsert({
            ...
            update: {
                ...
                isVerified: provider.status === client_1.ProviderStatus.APPROVED,
                averageRating: 4.5,
                cityId: city.id,
            },
            create: {
